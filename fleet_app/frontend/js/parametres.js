...
async function loadCamions() {
  try {
    const camions = await api.listCamions();
    if (!camions.length) {
      el("camionsBody").innerHTML = `<tr><td colspan="5" style="color:var(--text-muted);text-align:center;padding:24px;">Aucun camion enregistré pour l'instant.</td></tr>`;
      return;
    }
    el("camionsBody").innerHTML = camions.map((c) => `
      <tr>
        ...
        <td><button class="btn btn-secondary btn-sm" onclick='ouvrirEdition(${JSON.stringify(c.camion)})'>✎ Modifier</button></td>
      </tr>
    `).join("");
  } catch (err) {
    showConnError(err);
  ...
  try {
    ...
    el("utilisateursBody").innerHTML = utilisateurs.map((u) => `
      <tr>
        ...
        <td>${u.actif && u.identifiant !== session.getIdentifiant() ? `<button class="btn btn-secondary btn-sm" onclick="desactiverUtilisateur(${u.id})">Désactiver</button>` : "—"}</td>
      </tr>
    `).join("");
  } catch (err) {
    showConnError(err);
